...
//	else
//		philo_arr[i].second_fork = &fork_arr[0];
void	assign_neighbour_flag_and_mutexes(t_philo *philo_arr,
		t_shared *shared_args)
{
	...
	while (++i < shared_args->philo_max)
	{
		if (i != 0)
		{
			philo_arr[i].first_neighbour_mutex
				= &philo_arr[i - 1].shared_with_neighbours_mutex;
			philo_arr[i].first_neighbour_flag
				= &philo_arr[i - 1].first_forks_taken;
		}
		else
		{
			philo_arr[i].first_neighbour_mutex = &philo_arr
				[shared_args->philo_max - 1].shared_with_neighbours_mutex;
			philo_arr[i].first_neighbour_flag = &philo_arr
				[shared_args->philo_max - 1].first_forks_taken;
		}
		if (i + 1 != shared_args->philo_max)
		{
			philo_arr[i].second_neighbour_mutex
				= &philo_arr[i + 1].shared_with_neighbours_mutex;
			philo_arr[i].second_neighbour_flag
				= &philo_arr[i + 1].first_forks_taken;
		}
		else
		{
			philo_arr[i].second_neighbour_mutex
				= &philo_arr[0].shared_with_neighbours_mutex;
			philo_arr[i].second_neighbour_flag
				= &philo_arr[0].first_forks_taken;
		}
	}
}

int	init_state_arr(t_shared *shared_args)
{
	int	i;
	...
	{
		shared_args->state_arr = malloc(sizeof(bool) *
			(shared_args->philo_max / 2));
		if (!shared_args->state_arr)
			return (-1);
		while (++i < (shared_args->philo_max / 2))
			shared_args->state_arr[i] = false;
	}
	return (0);
}
*/
